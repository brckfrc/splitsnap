...
Only the physical device needs the beta toolchain. Every simulator target, on either iOS version, runs on stable Xcode:

| Target | Toolchain | Command |
|--------|-----------|---------|
| Whatever simulator Xcode picks by default | stable (`xcode-select` default) | `npm run ios` |
| iOS 26.5 simulator — `iPhone 17 Pro Max (26.5)` | stable | `npm run ios:26` |
| iOS 27.0 simulator — `iPhone 17 Pro Max` | stable | `npm run ios:27` |
| Pick from a prompt (any simulator or connected device) | stable | `npm run ios:pick` |
| Physical iPhone on the iOS 27 public beta | `/Applications/Xcode-beta.app` | `npm run ios:device` |

A device running an iOS **beta** can only be deployed to by an Xcode that ships that SDK and the matching device-support files, and the stable Xcode tops out at the iOS 26.5 SDK.

Simulators are the exception, and the distinction is worth being precise about because it is easy to get backwards: **the simulator's runtime version is independent of the SDK you compile against.** Stable Xcode builds with the iOS 26.5 SDK, and that binary runs happily on both the 26.5 and the 27.0 simulator runtimes — a lower SDK on a newer OS is the normal forward-compatibility case. So `ios:26` and `ios:27` differ only in which runtime the app is launched on, not in which compiler produced it, which also means alternating between them costs nothing: same toolchain, no Swift module mismatch, no cache to clear.

Testing on 26.5 matters because that is where most users are — a regression that only reproduces on the older OS is invisible if every simulator you own runs the beta.

> [!NOTE]
> Simulator **runtimes** and simulator **devices** are separate things. Having the iOS 26.5 runtime installed (Xcode → Settings → Components) does not give you a device on it; `xcrun simctl list devices` will show an empty `-- iOS 26.5 --` section until one is created. `npm run ios:26` targets a device created once with:
>
> ```bash
> xcrun simctl create "iPhone 17 Pro Max (26.5)" com.apple.CoreSimulator.SimDeviceType.iPhone-17-Pro-Max com.apple.CoreSimulator.SimRuntime.iOS-26-5
> ```
>
> Both scripts use **Pro Max** so the window matches the geometry the App Store screenshots are captured at (1320 × 2868 — see [`APP-STORE.md`](./APP-STORE.md)). The `(26.5)` suffix is load-bearing: Expo resolves `--device` by **exact** name or UDID match, so the 26.5 device must not be named `iPhone 17 Pro Max` like the 27.0 one.
...
"ios:26": "expo run:ios --device \"iPhone 17 Pro (26.5)\"",
"ios:27": "expo run:ios --device \"iPhone 17 Pro\"",
...
> [!WARNING]
> Plain `npx expo run:ios --device` is **not** equivalent, even if you pick the iOS 27 iPhone from the prompt — and neither is `npm run ios:pick`, which is that same command. Without `DEVELOPER_DIR` the build runs on the stable toolchain and its iOS 26.5 SDK, which cannot deploy to an iOS 27 device. The device still shows up in the picker (the connection list is toolchain-independent), so the failure comes later, during build or install. Use `ios:pick` to choose among **simulators**; for the physical iPhone always go through `npm run ios:device`.
